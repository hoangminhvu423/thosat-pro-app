...
def fill_in(m0,m1,trig,d):
    ...
    return None,None
HOLDOUT=datetime(2025,7,15)
def seg_of(dt):
    if dt>=HOLDOUT: return 'H'          # time-holdout: sau khi data XAU chuan ket thuc
    y=dt.year
    return 'P1' if y<2019 else ('P2' if y<2022 else 'P3')
...
GATES=[
 ('G2 ER>=0.35',  lambda g: er[g] is not None and er[g]>=0.35),   # DONG BANG tu R7c
 ('G0 random50',  lambda g: ((g*2654435761)>>4)&1==0),            # doi chung
]
def st(rs):
    n=len(rs)
    if n==0:return None
...
CONFIGS=[('PRIMARY',1.5,2,0.2,0.7)]   # DONG BANG
...
for cname,IMP,K,RLO,RHI in CONFIGS:
    ...
    base={}
    for sg in ['P1','P2','P3','H']:
        s=st([r for s_,r,_ in ts if s_==sg]); base[sg]=s
    print(f"{'gate':14} {'':4} "+" | ".join(f"{sg}: n avgR" for sg in ['P1<19','P2<22','P3<25.5','HOLDOUT'])+" | keep%")
    b=" | ".join(f"{(base[sg]['n'] if base[sg] else 0):4d} {(base[sg]['avg'] if base[sg] else 0):+.3f}" for sg in ['P1','P2','P3','H'])
    ...
    for gname,fn in GATES:
        ...
        for sg in ['P1','P2','P3','H']:
            ...
            if sg=='H' and so and so['avg']>=-0.02:dev_on_ok=True
